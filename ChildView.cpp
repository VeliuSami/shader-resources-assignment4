...
void CChildView::InitGL()
{
	...
	// Cat — environment map (replaces metallic sphere); mesh uses same shader pipeline as sphere2
	m_cat.LoadOBJ("models\\cat.obj");
	m_cat.m_program = LoadShaders("ShaderWnd/vertexSphere2.glsl", "ShaderWnd/fragmentSphere2.glsl");
	m_cat.InitGL();

	// Fish — diffuse texture (same shader family as bunny)
	...
	// Regular decagonal prism — diffuse texture (marble)
...
void CChildView::RenderGL()
{
	...
	// --- Fish (diffuse) — in front of the cat, toward the camera ---
